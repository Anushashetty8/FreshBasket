...
const { products, navigate, addToCart } = useContext(AuthContext);
const {id} = useParams();
...
useEffect(() => {
  if (product && Array.isArray(product.image) && product.image.length > 0) {
    setThumbnail(product.image[0]); // ✅ first image in array
  } else {
    setThumbnail(null);
  }
}, [product]);
    return product && (
        <div className="max-w-6xl w-full px-6">
            <p>
                <Link to={"/"}>Home</Link> /<Link to={"/productList"}> Products</Link> /
                ...
                <div className="flex gap-3">
                    <div className="flex flex-col gap-3">
                        {product.image.map((image, index) => (
                            <div key={index} onClick={() => setThumbnail(`http://localhost:5000/images/${image}`)}
